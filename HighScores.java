...
// Creates the High Scores UI and pulls High Scores from the database
import javax.swing.*;
...
//	}
	...
	public HighScores() {
		...
		int i = 1;
		while(getHighScores().size()>=i) {
			scoresPanel.add(new JLabel(getHighScores().get(0)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(1)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(2)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(3)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(4)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(5)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(6)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(7)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(8)));
			i++;

			scoresPanel.add(new JLabel(getHighScores().get(9)));
			i++;
	...
	public static ArrayList<String> getHighScores() {
		try {
			...
			HttpGet httpGet = new HttpGet("http://127.0.0.1:5000/getTopFive"); //  set the server url for the getEntity method
